    ...
    >
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        ...
        <div style={{ display: 'flex', gap: '12px' }}>
          <button onClick={() => { showToast('Public share link copied to clipboard!'); }} className="btn btn-outline">
            <Share2 size={16} /> Share Itinerary
          </button>
          <button onClick={() => setCurrentScreen(5)} className="btn btn-secondary">
            Edit Itinerary
          </button>
        </div>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '2.2fr 1fr', gap: '24px' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
          <div className="glass-card" style={{ padding: '24px', backgroundColor: '#ffffff' }}>
            <h3 style={{ fontSize: '1.2rem', fontWeight: 800, color: '#064e3b', marginBottom: '16px' }}>
              Day-by-Day Travel Schedule
            </h3>

            {selectedTrip.days.map((day) => (
              <div key={day.dayNum} style={{ marginBottom: '24px', borderBottom: '1px solid #f1f5f9', paddingBottom: '18px' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '12px' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                    ...
                    <h4 style={{ fontSize: '1.05rem', fontWeight: 800, color: '#0f172a' }}>{day.title}</h4>
                  </div>
                </div>
                  ...
                  {day.activities.map((act, idx) => (
                    ...
                    >
                      <div style={{ display: 'flex', alignItems: 'center', gap: '14px' }}>
                        <span style={{ fontWeight: 700, fontSize: '0.82rem', color: '#047857', width: '60px' }}>{act.time}</span>
                        <div>
                          <div style={{ fontWeight: 700, fontSize: '0.9rem', color: '#0f172a' }}>{act.title}</div>
                          <div style={{ fontSize: '0.76rem', color: '#64748b' }}>Category: {act.category}</div>
                        </div>
                      </div>

                      <div style={{ display: 'flex', alignItems: 'center', gap: '14px' }}>
                        <span className={`badge ${act.level === 'High' ? 'badge-gold' : 'badge-emerald'}`}>
                          Physical: {act.level}
                        </span>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
          ...
          {/* Budget Meter */}
          <div className="glass-card" style={{ padding: '24px', backgroundColor: '#ffffff' }}>
            <h3 style={{ fontSize: '1.1rem', fontWeight: 800, color: '#064e3b', marginBottom: '16px', display: 'flex', alignItems: 'center', gap: '8px' }}>
            ...
            <div style={{ textAlign: 'center', marginBottom: '20px' }}>
              ...
              {/* Progress bar */}
              <div style={{ width: '100%', height: '10px', backgroundColor: '#e2e8f0', borderRadius: '9999px', marginTop: '12px', overflow: 'hidden' }}>
                <div style={{ width: `${budgetRatio}%`, height: '100%', backgroundColor: budgetRatio > 90 ? '#ef4444' : '#064e3b', borderRadius: '9999px' }}></div>
              </div>
              <div style={{ fontSize: '0.78rem', fontWeight: 700, color: budgetRatio > 90 ? '#ef4444' : '#047857', marginTop: '6px' }}>
            ...
            {budgetRatio > 100 && (
              <div style={{ padding: '12px', borderRadius: '12px', backgroundColor: '#fef2f2', border: '1px solid #fca5a5', color: '#b91c1c', fontSize: '0.82rem', display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '16px' }}>
                <AlertTriangle size={18} />
                <span><strong>Overbudget Alert:</strong> Trip expenses exceed estimated target budget by ₹{(totalSpent - selectedTrip.estimatedBudget).toLocaleString()}!</span>
              </div>
            )}
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '12px', fontSize: '0.86rem' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                <span style={{ color: '#475569', display: 'flex', alignItems: 'center', gap: '6px' }}><Plane size={15} color="#3b82f6" /> Flights</span>
                <strong style={{ color: '#0f172a' }}>₹{selectedTrip.categoryBreakdown.flights.toLocaleString()}</strong>
              </div>
              <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                <span style={{ color: '#475569', display: 'flex', alignItems: 'center', gap: '6px' }}><Building2 size={15} color="#6366f1" /> Hotels</span>
                <strong style={{ color: '#0f172a' }}>₹{selectedTrip.categoryBreakdown.hotels.toLocaleString()}</strong>
              </div>
              <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                <span style={{ color: '#475569', display: 'flex', alignItems: 'center', gap: '6px' }}><UtensilsCrossed size={15} color="#10b981" /> Food</span>
                <strong style={{ color: '#0f172a' }}>₹{selectedTrip.categoryBreakdown.food.toLocaleString()}</strong>
              </div>
              <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                <span style={{ color: '#475569', display: 'flex', alignItems: 'center', gap: '6px' }}><Compass size={15} color="#f59e0b" /> Activities</span>
                <strong style={{ color: '#0f172a' }}>₹{selectedTrip.categoryBreakdown.activities.toLocaleString()}</strong>
              </div>
              <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                <span style={{ color: '#475569', display: 'flex', alignItems: 'center', gap: '6px' }}><Bus size={15} color="#8b5cf6" /> Transport</span>
                <strong style={{ color: '#0f172a' }}>₹{selectedTrip.categoryBreakdown.transport.toLocaleString()}</strong>
              </div>
            </div>

            <div style={{ marginTop: '20px', padding: '12px', borderRadius: '12px', backgroundColor: '#f8fafc', border: '1px solid #e2e8f0', textAlign: 'center', fontSize: '0.82rem', color: '#475569' }}>
